...
struct PlainArena : public PlainSpace
{
    ...
    PlainArena(int _id, unsigned _gate = DIR_NONE) : PlainSpace(_gate), id(_id), isStart(false), isEnd(false)
    {
    }
};
...
struct SpaceAttribute
{
    ...
    SpaceAttribute()
        : rounds(0), isDiscovered(false), isCompleted(false), isCurrent(false), isStart(false), isEnd(false),
          hasBoss(false), showCreate(false)
    {
    }
...
public:
    ...
    void Generate(PlainSpace* spc) override;
...
public:
    ...
    {
        return m_graph.Width();
    }

    int GetGraphHeight() const
    {
        return m_graph.Height();
    }
