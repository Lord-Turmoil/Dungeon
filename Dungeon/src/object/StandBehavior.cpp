...
{
    static_cast<Stand*>(m_parent->GetGameObject())->GetSymbol()->SetSubSymbol(nullptr);
}

void StandBehavior::_AdjustDirection(bool isLeft)
{
    auto anim = m_parent->GetGameObject()->GetComponent<AnimComponent>()->GetAnim();
...
void StandFlash::_GeneratePortal()
{
    ...
    // Flashback* flashback = stand->GetFlashback();
    Dungeon* dungeon = static_cast<Dungeon*>(stand->GetScene());
    Portal* portal = MiscLibrary::GetInstance()->GetMiscObject<Portal>("Portal")->Clone();
