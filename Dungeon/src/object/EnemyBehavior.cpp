...
void EnemyBehavior::_Evade(int level)
{
    ...
    const double A = static_cast<double>(level) * 8.0;
    const double B = static_cast<double>(level + 1) * 4.0;
...
void EnemyInit::OnEnter()
{
    ...
    enemy->GetComponent<RigidBodyComponent>()->SetID(COLL_ID_SPECTER);
}
...
void EnemyInit::OnExit()
{
    ...
    enemy->GetComponent<RigidBodyComponent>()->SetID(COLL_ID_ENEMY);
}
...
void EnemyIdle::OnEnter()
{
    ...
    enemy->GetComponent<AnimComponent>()->GetAnim()->SetMotion(ENEMY_ANIM_IDLE);
...
void EnemyAttack::Update(Event* evnt)
{
    ...
    if (enemy->GetComponent<AnimComponent>()->GetAnim()->IsOver())
    {
...
void EnemyAttack::OnEnter()
{
    ...
    enemy->GetComponent<AnimComponent>()->GetAnim()->SetMotion(ENEMY_ANIM_ATTACK);
...
{
    m_parent->GetGameObject()->GetComponent<AnimComponent>()->GetAnim()->SetMotion(ENEMY_ANIM_IDLE);

    m_elapsedTime = 0L;

    clock_t duration = static_cast<Enemy*>(m_parent->GetGameObject())->GetRestDuration();
    m_duration = Random(static_cast<int>(duration << 1), static_cast<int>(duration >> 1));
}
...
void EnemyDead::OnEnter()
{
    Enemy* enemy = static_cast<Enemy*>(m_parent->GetGameObject());
    Dungeon* dungeon = static_cast<Dungeon*>(enemy->GetScene());
    enemy->GetComponent<AnimComponent>()->GetAnim()->SetMotion(ENEMY_ANIM_DEAD);
    enemy->GetComponent<RigidBodyComponent>()->SetID(COLL_ID_SPECTER);
