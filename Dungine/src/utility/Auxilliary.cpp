...
{
	return ((rect.left <= target.x) && (target.x <= rect.right) \
		&& (rect.top <= target.y) && (target.y <= rect.bottom));
}
...
bool InRectangle(const Rect& rect, const Coordinate& target)
{
	return ((rect.pos.x <= target.x) && (target.x < rect.pos.x + rect.width) \
		&& (rect.pos.y <= target.y) && (target.y < rect.pos.y + rect.height));
}

bool InRectangleStrict(const Rect& rect, const Coordinate& target)
{
	return ((rect.pos.x < target.x) && (target.x < rect.pos.x + rect.width - 1) \
		&& (rect.pos.y < target.y) && (target.y < rect.pos.y + rect.height - 1));
}
